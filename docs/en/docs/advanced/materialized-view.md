...
In order to ensure the data consistency between the materialized view table and the Base table, Doris synchronizes operations such as import and delete to the materialized view tables. And through incremental update to improve update efficiency. To ensure atomicity through transaction.
